	...
	function SlickGrid(container, data, columns, options) {
		...
		function getColumnCssRules(idx) {
			if (!stylesheet) {
				...
				for (var i = 0; i < cssRules.length; i++) {
					...
					var matches = /\.l\d+/.exec(selector);

					if (matches) {
						var columnIdx = parseInt(matches[0].substr(2), 10);
		...
		function cleanupRows(rangeToKeep) {
			for (var key in rowsCache) {
				if (rowsCache.hasOwnProperty(key)) {
					var rowIndex = parseInt(key, 10);
					if (rowIndex !== activeRow && (rowIndex < rangeToKeep.top || rowIndex > rangeToKeep.bottom)) {
		...
		function cleanUpCells(range, row) {
			...
			for (var i in cacheEntry.cellNodesByColumnIdx) {
				...
				if (columnPosLeft[i] > range.rightPx ||
					columnPosRight[Math.min(Number(columns.length - 1), (+i) + (+colspan) - 1)] < range.leftPx) {
